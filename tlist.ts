...
const App: React.FC = () => {
   ...
   */
  const handleAddTask = () => {
    if (newTask.trim()) { // Ensure the input is not empty
  ...
  return (
    ...
    <div className="min-h-screen bg-black text-white flex flex-col font-inter">
      ...
      <main className="flex-grow flex flex-col md:flex-row px-4 md:px-8 gap-6 pb-24">
        ...
        >
          <div className="bg-[#1a3a3a] bg-opacity-80 rounded-t-2xl p-4 flex justify-between items-center border-b border-[#2a4a4a]">
            ...
            <div className="flex gap-2"> {/* Container for sort buttons */}
              <button
          ...
          <div className="p-4 flex-grow space-y-3 overflow-y-auto custom-scrollbar">
            {getTasksByColumn("completed").map((task) => (
              ...
              >
                ...
                <span className={`ml-4 px-2 py-1 text-xs font-bold rounded-md text-white ${priorityColors[task.priority]}`}>{task.priority}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      >
        <i
          className={`fas fa-trash-alt text-2xl ${
            isDraggingOverBin ? "text-white" : "text-blue-300"
          }`}
        ></i>
      </div>
